...
country_coordinates <- read_csv("country_codes/countries_codes_and_coordinates.csv") %>% 
  clean_names() %>% 
  select(country, code = 'alpha_2_code', "latitude_average","longitude_average")
...
## World Map of Imports of Popular SPecies

```{r}
## interactive map of popular imports of species
tmap_mode(mode = 'view') #toggles to interactive viewing
tm_shape(world_import_sf) + #taking a shape..
  tm_fill('import_count', palette = 'BuGn') + #attributing a fill to that shape
  tm_borders(col = 'black') +
  tm_shape(top_species_imports) +
  tm_dots(col= "taxon",
          size = 0.05,
          popup.vars = c("country:" = "country", "species:" = "taxon","imported count" = "n"),
          title = "Imported Taxon")
```

## World Map of Exports of Popular Species

```{r}
## map of exports
tmap_mode(mode = 'view') #toggles to interactive viewing
tm_shape(import_export_sf) + #taking a shape..
  tm_fill('export_count', palette = 'BuGn') + #attributing a fill to that shape
  tm_borders(col = 'black') +
  tm_shape(top_species_exports) +
  tm_dots(col= "taxon",
          size = 0.05,
          popup.vars = c("country:" = "country", "species:" = "taxon","exported count" = "n"),
          title = "Exported Taxon")
```
